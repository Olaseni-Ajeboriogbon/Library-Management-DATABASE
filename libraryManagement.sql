...
/* ======================= TABLES ========================*/
	...
	CREATE TABLE tbl_book_loans (
		...
		book_loans_BookID INT NOT NULL CONSTRAINT fk_book_id1 FOREIGN KEY REFERENCES table_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
		book_loans_BranchID INT NOT NULL CONSTRAINT fk_branch_id1 FOREIGN KEY REFERENCES table_library_branch(library_branch_BranchID) ON UPDATE CASCADE ON DELETE CASCADE,
		book_loans_CardNo INT NOT NULL CONSTRAINT fk_cardno FOREIGN KEY REFERENCES table_borrower(borrower_CardNo) ON UPDATE CASCADE ON DELETE CASCADE,
	...
	CREATE TABLE tbl_book_copies (
		...
		book_copies_BookID INT NOT NULL CONSTRAINT fk_book_id2 FOREIGN KEY REFERENCES table_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
		book_copies_BranchID INT NOT NULL CONSTRAINT fk_branch_id2 FOREIGN KEY REFERENCES table_library_branch(library_branch_BranchID) ON UPDATE CASCADE ON DELETE CASCADE,
	...
	CREATE TABLE tbl_book_authors (
		...
		book_authors_BookID INT NOT NULL CONSTRAINT fk_book_id3 FOREIGN KEY REFERENCES table_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
...
/*==================== POPULATING TABLES ======================*/
	...
	SELECT * FROM table_book WHERE book_PublisherName = 'George Allen & Unwin'

	INSERT INTO table_library_branch
...
alter procedure dbo.BooksOwnedByBranch
@book_titled nvarchar(30),@library_branch nvarchar(30)
as
Begin
select table_book.book_BookID as BookID,table_book.book_Title as Book_Title,
tbl_book_copies.book_copies_No_of_copies as Number_of_copies,
table_library_branch.library_branch_BranchName as Branch_name
from table_book 
join 
tbl_book_copies
on 
table_book.book_BookID = tbl_book_copies.book_copies_BookID
join 
table_library_branch
on 
library_branch_branchID = tbl_book_copies.book_copies_BranchID
where 
@book_titled = table_book.book_Title 
and 
@library_branch = table_library_branch.library_branch_BranchName
End;

exec dbo.BooksOwnedByBranch 'The Lost Tribe' , 'sharpstown';

CREATE PROCEDURE dbo.bookCopiesAtAllSharpstown 
@bookTitle varchar(70), @branchName varchar(70) 
AS
...
GO
EXEC dbo.bookCopiesAtAllSharpstown @booktitle = 'The Lost Tribe',@branchName = 'sharpstown'


/* #2- How many copies of the book titled "The Lost Tribe" are owned by each library branch? */

Alter procedure HowManyBookCopies
@booktitled nvarchar(30)
as
Begin
Select table_library_branch.library_branch_BranchID as Branch_Id,table_library_branch.library_branch_BranchName as BranchNAme, 
table_book.book_Title,sum (tbl_book_copies.book_copies_No_Of_Copies) as No_of_copies
from table_library_branch
join tbl_book_copies 
on table_library_branch.library_branch_BranchID =tbl_book_copies.book_copies_BranchID
join table_book
on tbl_book_copies.book_copies_BookID = table_book.book_BookID
where @booktitled = table_book.book_Title
group by table_library_branch.library_branch_BranchID,table_library_branch.library_branch_BranchName,table_book.book_Title
End

Exec HowManyBookCopies 'The Lost Tribe'


/* #3- Retrieve the names of all borrowers who do not have any books checked out. */
CREATE PROCEDURE LoanerswithoutBooks
AS
BEGIN
SELECT table_borrower.borrower_CardNo As Card_No, table_borrower.borrower_BorrowerName As BorrowersName
from table_borrower where borrower_CardNo not in(select tbl_book_loans.book_loans_CardNo from tbl_book_loans)
END

EXEC LoanerswithoutBooks

/*---------------------------Alternatively-----------------------------------------------------------------*/



alter proc BorrowersWithoutBooks
as
Begin
select table_borrower.borrower_CardNo As CardNo, table_borrower.borrower_BorrowerName As BorrowersName from
table_borrower where Not Exists
(Select * from tbl_book_loans where table_borrower.borrower_CardNo = tbl_book_loans.book_loans_CardNo)
End

Exec BorrowersWithoutBooks
		

/* #4- For each book that is loaned out from the "Sharpstown" branch and whose DueDate is today, retrieve the book title, the borrower's name, and the borrower's address.  */
CREATE PROC Retrival
@Booktitle nvarchar(30),@borrowersName nvarchar(30),@borrowersAdd nvarchar(30)
AS
BEGIN
SELECT
END
...
SELECT Branch.library_branch_BranchName AS [Branch Name],  Book.book_Title [Book Name],
	   ...
	   FROM tbl_book_loans AS Loans
			INNER JOIN table_book AS Book ON Loans.book_loans_BookID = Book.book_BookID
...
AS
	SELECT Borrower.borrower_BorrowerName AS [Borrower Name], Borrower.borrower_BorrowerAddress AS [Borrower Address],
		   ...
		   FROM tbl_book_loans AS Loans
		   			INNER JOIN table_borrower AS Borrower ON Loans.book_loans_CardNo = Borrower.borrower_CardNo
...
AS
	SELECT Branch.library_branch_BranchName AS [Branch Name], Book.book_Title AS [Title], Copies.book_copies_No_Of_Copies AS [Number of Copies]
		   FROM tbl_book_authors AS Authors
				INNER JOIN table_book AS Book ON Authors.book_authors_BookID = Book.book_BookID
				INNER JOIN tbl_book_copies AS Copies ON Authors.book_authors_BookID = Copies.book_copies_BookID
				INNER JOIN table_library_branch AS Branch ON Copies.book_copies_BranchID = Branch.library_branch_BranchID
